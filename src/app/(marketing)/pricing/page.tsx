...
const plans = [
  {
    ...
    features: ['Up to 5 users', '500 documents', 'RAG Q&A with citations', 'PDF, DOCX, CSV support', 'Email support'],
    cta: 'Get started',
    highlight: false,
  },
  {
    ...
    features: ['Up to 50 users', 'Unlimited documents', 'Role-based access control', 'Risk & trend detection', 'Executive dashboards', 'Priority support'],
    cta: 'Book a demo',
    highlight: true,
  },
  {
    ...
    features: ['Unlimited users', 'Unlimited documents', 'Audit logs & governance', 'Custom integrations', 'SSO / SAML', 'Dedicated success manager', 'SLA guarantee'],
    cta: 'Contact us',
    highlight: false,
  },
]
        ...
        {plans.map((plan) => (
          <div
            ...
            className={`flex flex-col rounded-2xl border p-8 ${
              plan.highlight ? 'border-brand bg-brand text-white shadow-xl' : 'border-gray-200 bg-white'
            }`}
          >
            ...
            <p className={`mt-3 text-sm ${plan.highlight ? 'text-blue-100' : 'text-gray-500'}`}>{plan.description}</p>

            <ul className="mt-8 flex-1 space-y-3">
              {plan.features.map((f) => (
                <li key={f} className="flex items-start gap-2 text-sm">
                  <CheckCircle2 className={`mt-0.5 h-4 w-4 shrink-0 ${plan.highlight ? 'text-gold' : 'text-brand'}`} />
